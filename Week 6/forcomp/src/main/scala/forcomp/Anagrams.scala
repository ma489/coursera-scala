...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.toList.groupBy(c => c).map(c => (c._1, c._2.size)).toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    ...
    else if (occurrences.head._2 <= 0) List(List())
    else {
      combinations(occurrences.tail) ++
        combinations(occurrences.tail).map(o => occurrences.head :: o) ++
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val xMap = x.toMap
    val yMap = y.toMap
    x.diff(y).map(ci => (ci._1, ci._2 - yMap.getOrElse(ci._1, 0))).sorted
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def aux(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
        ...
        sentence <- aux(subtract(occurrences, wordOccurrences(word)))
      } yield word :: sentence
    }
